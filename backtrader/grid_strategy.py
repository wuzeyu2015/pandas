...
class GridStrategy(bt.Strategy):
    ...
    params = (
        ...
        ('initial_position', -1),       # 【初始持仓】回测开始时的初始持仓数量（股数）
    )
    ...
    def _validate_params(self):
        ...
        if self.params.initial_position < 0:
            raise ValueError("必须指定 position_min 参数（最小持仓）")
    ...
    def notify_order(self, order):
        ...
        if order.status in [order.Completed]:
            if order.isbuy():
                first_day_close = bt.num2date(order.created.dt).date() == self.first_date
                if not first_day_close :
                    self.buys += 1
                self.log(
                    f'✅ {"建仓买入" if first_day_close else "买入"}成交 | 价格: {order.executed.price:.3f} | '
    ...
    def next(self):
        ...
        if self.first_date is None:
            self.first_date = self.datas[0].datetime.date(0)
        ...
        if self.first_date == current_date:
            return
    ...
    def _initialize_base_price(self):
        ...
        # 🔍 调试日志：显示当前是哪个时间点
        self.log(f'🔍 [DEBUG] 初始化基准价为当天开盘价 - 时间: {current_dt}, 开盘价: {open_price:.3f}', doprint=True)
    ...
    def _update_base_price_at_end_of_day(self):
        ...
        if bar_end_time.time() == market_close:
            ...
            # self.log(
            #     f'📅 盘尾更新基准价: '
            #     f'{old_base:.3f} -> {self.base_price:.3f}'
            #     if old_base is not None
            #     else f'📅 首次初始化基准价: {self.base_price:.3f}',
            #     doprint=True
            # )
            if self.first_date == bar_end_time.date():
                self.order = self.buy(size=self.params.initial_position)
                self.log(f'📉 建仓买入提交！触发价: {self.base_price:.3f}', doprint=True)
    ...
    def _execute_grid_trading(self, is_open_session=True):
        ...
        Args:
            is_open_session: 是否为开盘时段 (保留参数以兼容调用，目前内部逻辑统一)
        """
        ...
        while 1:
            # 检查买入条件
            if price <= self.base_price - self.params.grid_size:
                if self._execute_buy(simulated_position):
                    # ✅ 更新模拟持仓
            ...
            # 检查卖出条件
            elif price >= self.base_price + self.params.grid_size:
                if self._execute_sell(simulated_position):
                    # ✅ 更新模拟持仓
                    simulated_position -= self.params.trade_shares
                else:
                    break
    ...
    def _execute_buy(self, simulated_position):
        """
        执行买入操作
        ...
        self.log(f'📉 买入提交！触发价: {buy_trigger:.3f}, 更新基准价: {old_base:.3f} -> {self.base_price:.3f}', doprint=True)
    ...
    def _execute_sell(self, simulated_position):
        """
        执行卖出操作
        ...
        self.log(f'📈 卖出提交！触发价: {sell_trigger:.3f}, 更新基准价: {old_base:.3f} -> {self.base_price:.3f}', doprint=True)
...
def run_backtest(
    ...
    **strategy_kwargs
    ...
    try:
        ...
        print(f"📈 佣金费率: {commission*100:.3f}%")
